...
const Controls = ({ types }) => (
  <ControlWrap className="column is-one-fifth">
    <ControlBox>
      <Control>
        <h6 style={{ display: "flex", padding: "5px 10px" }}>
          Galleries
          <a style={{ marginLeft: "auto" }} href="/" className="clear">
            Clear
          </a>
        </h6>
        {types.map(type => (
          <Checkbox key={slugify(type)} name="type" value={type} />
        ))}
        <br />
        <Button value={"Select"} />
      </Control>
...
const gallery = ({ data, location }) => {
  ...
  const types = aggregate(data.event.edges, {
    column: "type",
    property: "label"
  });
              ...
              data={data.event.edges.map(edge => {
                ...
                edge.node.frontmatter.cardDescription = (
                  <span class="event">
                    ...
                    <br />
                    <span class="date">{edge.node.frontmatter.date}</span>
                  </span>
                );
                return edge;
              })}
