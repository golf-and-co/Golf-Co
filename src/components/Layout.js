...
const TemplateWrapper = ({ children }) => (
  <StaticQuery
    query={graphql`
      {
        allMarkdownRemark(
          filter: { frontmatter: { templateKey: { eq: "meta" } } }
        ) {
          edges {
            node {
              frontmatter {
    ...
    `}
    render={data => {
      const metatags = data.allMarkdownRemark.edges[0].node.frontmatter.meta.map(
        tag => {
      ...
      return (
        <div className="container columns is-fluid no-margin">
          <Helmet>
            <html lang="en" />
            <title>
              {data.allMarkdownRemark.edges[0].node.frontmatter.siteTitle}
            </title>
            ...
            <link
              ...
              sizes="180x180"
              href={
                data.allMarkdownRemark.edges[0].node.frontmatter.favicon
                  .publicURL
              }
            />
            <link
            ...
            <script
              type="text/javascript"
              src="//s7.addthis.com/js/300/addthis_widget.js#pubid=ra-5caa087d1b5659b0"
            />
          </Helmet>
